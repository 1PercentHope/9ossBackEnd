...
const updateEventInfo = (info) => {
  let syntax = `UPDATE events SET homeTeam= '${homeTeam}, awayTeam= '${info.awayTeam}', place='${info.place}', category='${info.category}', date = '${info.date}', description = '${info.description}', image= '${info.image} 'WHERE id =${info.id}`;
  return new Promise((resolve, reject) => {
    db.connection.query(syntax, (error, results) => {
      if (error) {
        reject(error);
